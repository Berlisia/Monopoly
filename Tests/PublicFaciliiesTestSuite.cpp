...
void PropertyPublicFacilitiesTestSuite::
    setupPropertyWithPublicFacilitesPayMode() {
  auto waterSupply = std::make_unique<Property>(
      WATER_SUPPLY_PRICE, std::make_unique<PublicFacilities>(district),
      district, WATER_SUPPLY_NAME, bankier);
  auto powerStation = std::make_unique<Property>(
      POWER_STATION_PRICE, std::make_unique<PublicFacilities>(district),
      district, POWER_STATION_NAME, bankier);
...
TEST_F(PropertyPublicFacilitiesTestSuite,
       dawidShouldPayRentForMarekWhenMarekHaveOnlyPowerStation) {
  ...
  const auto expectedMoneyForMarek =
      moneyOnStartGame - POWER_STATION_PRICE + expectedRent;
...
TEST_F(PropertyPublicFacilitiesTestSuite,
       dawidShouldPayRentForMarekWhenMarekHaveTwoPublicFacilities) {
  unsigned int steps = 1;
  ...
  EXPECT_CALL(dice, diceThrow())
      ...
      WillOnce(
          ::testing::Return(rollDiceSecondTurn)); // Dawid roll dice to pay rent
  ...
  auto expectedRentFirstTurn =
      FACTOR_OWNER_HAVE_ONE_FACILITY * rollDiceFirstTurn;
  auto expectedRentSecondTurn =
      FACTOR_OWNER_HAVE_TWO_FACILITY * rollDiceSecondTurn;

  auto statusMarek = playerFirst->status();
  auto statusDawid = playerSecond->status();

  const auto expectedMoneyForMarek =
      moneyOnStartGame - POWER_STATION_PRICE + expectedRentFirstTurn -
      WATER_SUPPLY_PRICE + expectedRentSecondTurn;
  EXPECT_EQ(statusMarek.money(), expectedMoneyForMarek);

  const auto expectedMoneyForDawid =
      moneyOnStartGame - expectedRentFirstTurn - expectedRentSecondTurn;
  EXPECT_EQ(statusDawid.money(), expectedMoneyForDawid);
}
