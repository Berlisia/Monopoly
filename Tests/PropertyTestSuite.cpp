...
public:
  PropertyTestSuite() {
    ...
    playerFirst = std::make_unique<Player>(
        "tester Marek", BoardIterator(propertisSut.begin(), propertisSut.end()),
        dice, subjectBuildingProperty);
    playerSecond = std::make_unique<Player>(
        "tester Dawid", BoardIterator(propertisSut.begin(), propertisSut.end()),
        dice, subjectBuildingProperty);
  }
...
void PropertyTestSuite::setupTestBoard() {
  ...
  auto buildingModeForPub = std::make_unique<BuildingProperty>(
      CardInfo{PUB_RENT, PUB_RENT_BUILDING, HOUSE_PRICE, HOTEL_PRICE,
               PUB_PRICE / 2},
      districts[0]);
  auto buildingModeForBear = std::make_unique<BuildingProperty>(
      CardInfo{BEAR_RENT, BEAR_RENT_BUILDING, HOUSE_PRICE, HOTEL_PRICE,
               BEAR_PRICE / 2},
      districts[0]);
  auto buildingModeForExpensive = std::make_unique<BuildingProperty>(
      CardInfo{EXPENSIVE_RENT, EXPENSIVE_RENT_BUILDING, HOUSE_PRICE,
               HOTEL_PRICE, EXPENSIVE_PRICE / 2},
      districts[1]);
  ...
  auto propertyPub =
      std::make_unique<Property>(PUB_PRICE, std::move(buildingModeForPub),
                                 districts[0], PUB_NAME, bankier);
  auto propertyBear =
      std::make_unique<Property>(BEAR_PRICE, std::move(buildingModeForBear),
                                 districts[0], BEAR_NAME, bankier);
  auto propertyExpensive = std::make_unique<Property>(
      EXPENSIVE_PRICE, std::move(buildingModeForExpensive), districts[1],
      EXPENSIVE_NAME, bankier);

  districts[0].assignPropertisToDistrict(
      {propertyPub.get(), propertyBear.get()});
...
TEST_F(PropertyTestSuite,
       playerSecondShouldPayRentForPlayerFirst_OnePropertyInDistrict) {
  ...
  const auto expectedMoneyForPlayerFirst =
      moneyOnStartGame - BEAR_PRICE + BEAR_RENT;
...
TEST_F(PropertyTestSuite,
       playerSecondShouldPayRentx2ForPlayerFirst_AllPropertyInDistrict) {
  unsigned int steps = 1;
  ...
  const auto expectedMoneyForPlayerFirst =
      moneyOnStartGame - BEAR_PRICE - PUB_PRICE + BEAR_RENT + (PUB_RENT * 2);
  EXPECT_EQ(statusPlayerFirst.money(), expectedMoneyForPlayerFirst);

  const auto expectedMoneyForPlayerSecond =
      moneyOnStartGame - BEAR_RENT - (PUB_RENT * 2);
  EXPECT_EQ(statusPlayerSecond.money(), expectedMoneyForPlayerSecond);
}

TEST_F(PropertyTestSuite,
       DawidShouldPayRentWhenMarekHaveTwoHousesOnBearProperty) {
  unsigned int steps = 1;
  ...
  playerFirst->turn();
  buildingModes[pubPropertyBuildingMode]->buyHouse(numberOfHousesToBuy,
                                                   *playerFirst);
  playerSecond->turn();
  ...
  const auto expectedMoneyForDawid =
      moneyOnStartGame - BEAR_RENT - PUB_RENT_BUILDING.at(numberOfHousesToBuy);
  EXPECT_EQ(statusDawid.money(), expectedMoneyForDawid);
}

TEST_F(PropertyTestSuite,
       DawidShouldPayRentx2WhenMarekHaveTwoHousesOnBearPropertyAndSellIt) {
  unsigned int steps = 1;
  unsigned int stepsInSecondTrun = 2;
  unsigned int pubPropertyBuildingMode = 0;
  unsigned int numberOfHousesToBuy = 2;
  ...
  playerFirst->turn();
  buildingModes[pubPropertyBuildingMode]->sellHouse(numberOfHousesToBuy,
                                                    *playerFirst);
  playerSecond->turn();
