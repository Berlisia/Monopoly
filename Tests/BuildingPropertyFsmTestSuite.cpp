...
TEST_F(BuildingPropertyFsmMortgage, ShouldPayOffDeptWhenReliveMortgageAndHaveAllPropertis)
{
    ...
    EXPECT_CALL(owner, withdrawMoney(morgagePrice)).WillOnce(testing::Return(true));
...
TEST_F(BuildingPropertyFsmMortgage, ShouldPayOffDeptWhenReliveMortgageAndHaveNotAllPropertis)
{
    ...
    auto badSize = 2;
    EXPECT_CALL(owner, withdrawMoney(morgagePrice)).WillOnce(testing::Return(true));
    EXPECT_CALL(owner, checkPropertisInDistrict(district.propertis())).WillOnce(testing::Return(badSize));
