...
const RentAtNumberOfBuildings RENT_BUILDING = {
...
std::unique_ptr<Property> createBuildingProperty(
    CardInfo card, unsigned int price, const std::string name,
    const District &district, SubjectBuildingProperty &subjectBuildingProperty,
    std::vector<HouseDevelop *> &buildingModes, Bankier &bankier) {
  ...
  return std::make_unique<Property>(price, std::move(buildingMode), district,
                                    name, bankier);
}

Squers createSimpleBoard(std::vector<District> &districts,
  ...
  auto property = createBuildingProperty(
      {10, RENT_BUILDING, HOUSE_PRICE, HOTEL_PRICE}, 100, "niedzwiedzia buda",
      districts[0], buildingPropertyNotify, buildingModes, bankier);
  ...
  property = createBuildingProperty(
      {20, RENT_BUILDING, HOUSE_PRICE, HOTEL_PRICE}, 100, "Stara hata",
      districts[0], buildingPropertyNotify, buildingModes, bankier);
  districts[0].assignPropertyToDistrict(property.get());
  squares.push_back(std::move(property));
  ...
  property = createBuildingProperty(
      {30, RENT_BUILDING, HOUSE_PRICE, HOTEL_PRICE}, 200, "Kocia kolyska",
      districts[1], buildingPropertyNotify, buildingModes, bankier);
  ...
  property = createBuildingProperty(
      {40, RENT_BUILDING, HOUSE_PRICE, HOTEL_PRICE}, 300, "Karczma 7 kotow",
      districts[1], buildingPropertyNotify, buildingModes, bankier);
  districts[1].assignPropertyToDistrict(property.get());
  squares.push_back(std::move(property));
  createRandomSquare(squares);
...
Players
createSomePlayers(BoardIterator board, const Dice &dice,
                  const SubjectBuildingProperty &buildingPropertyNotify) {
  Players players;
  players.push_back(
      std::make_unique<Player>("JANEK", board, dice, buildingPropertyNotify));
...
int main(int argc, char *argv[]) {
  ...
  auto squares = createSimpleBoard(districts, buildingModes,
                                   buildingPropertyNotify, bankier);
