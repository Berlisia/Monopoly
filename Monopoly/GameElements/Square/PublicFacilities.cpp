...
void PublicFacilities::payRent(Guest &player) {
  auto ownerHaveSecondFacility =
      owner->checkPropertisInDistrict(district.propertis());
...
void PublicFacilities::setNewOwner(Guest &newOwner) { owner = &newOwner; }

unsigned int
PublicFacilities::calculateRent(unsigned int ownerHaveSecondFacility,
                                unsigned int numberOfDice) const {
