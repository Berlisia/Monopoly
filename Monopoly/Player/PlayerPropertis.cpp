...
unsigned int PlayerPropertis::countPropertisInDistrict(
...
bool PlayerPropertis::findProperty(const Estate *property) {
  return std::find_if(propertis.begin(), propertis.end(),
                      [property](const Estate *playerProperty) {
                        return *property == *playerProperty;
                      }) != propertis.end();
}

std::vector<const Estate *>::iterator
PlayerPropertis::getProperty(const Estate *property) {
  return std::find_if(propertis.begin(), propertis.end(),
                      [property](const Estate *playerProperty) {
                        return *property == *playerProperty;
                      });
