...
Player::Player(std::string p_name, BoardIterator p_boardIterator,
    ...
    : name(p_name), actualPossisionOnBoard(p_boardIterator),
      propertis(p_buildingProperty), dice(p_dice) {
...
void Player::addMoney(unsigned int moneyToAdd) { money += moneyToAdd; }

bool Player::buyProperty(unsigned int price, const Estate *property,
...
void Player::sellProperty(unsigned int price, const Estate *property,
                          const District &district) {
  money += price;
  propertis.remove(property, district);
}

unsigned int Player::checkPropertisInDistrict(
    const std::vector<const Estate *> &propertisInDistrict) {
  return propertis.countPropertisInDistrict(propertisInDistrict);
}

unsigned int Player::rollDice() { return dice.diceThrow(); }
...
bool Player::haveEnoughtMoney(unsigned int price) { return price < money; }

void Player::printStatus() { std::cout << "Result: " << money << std::endl; }

const PlayerStatus Player::status() {
  return PlayerStatus{actualPossisionOnBoard, money,
                      propertis.snapchotPropertis()};
}

const std::string &Player::myName() { return name; }

void Player::stateTransition(std::unique_ptr<State> state) {
  currentState = std::move(state);
}

void Player::moveNextSquare() { ++actualPossisionOnBoard; }

Square *Player::actualSquare() { return (*actualPossisionOnBoard).get(); }
...
void Player::actionOnStop() { actualSquare()->actionOnStop(*this); }
